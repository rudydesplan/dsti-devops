...
class AvocadoPrep:
    def __init__(self, dataset_location: str = None, dataframe: DataFrame = None):
        ...
        if (dataset_location is not None and dataframe is not None) or (
            dataset_location is None and dataframe is None
        ):
        ...
        if dataset_location is not None:
            try:
                ...
                raise Exception(
                    f"Failed reading csv file, make sure the path is correct. {e}"
                )
            ...
            raise Exception(
                f"Error reading dataframe. Input schema must be same as: {AVOCADO_INPUT_COLUMNS}"
            )
    ...
    def add_small_plu(self):
        ...
        self.df["small_plu"] = self.dataset.apply(
            lambda row: min(row["4046"], row["4225"], row["4770"]), axis=1
        )

    def add_average_size_bags(self):
        self.df["average_size_bags"] = self.dataset.apply(
            lambda row: round(float(row["total_bags"]) / 3, 2), axis=1
        )
