...
# Prepare the dataset and insert/update the data in the MongoDB database.
...
# Prepare only one row of the csv dataset
# todo: replace index by unique_id, requires adding unique_id as meta_data to the csv file
@app.route("/prepare/<index>")
def prepare_row(index):
    ...
    if index in range(data.shape[0]):
        ...
        abort(404, description="row not found in csv")


# Delete an avocado document using a unique ID
...
# Get the total number of avocado documents in the collection
...
# Get all avocado documents from the 'avocados' collection
...
# Get a single document by its unique index from the avocados collection
...
# Get avocados for a specific season
...
# Get the average size bags for a specific region
...
# Get the count of avocados with a specific small PLU code
...
# Get avocado documents by state
...
# Get the count of avocado documents for a specific season and region
